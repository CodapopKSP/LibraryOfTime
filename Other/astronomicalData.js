...
function calculateDateFromJDE(JDE) {
  ...
  const totalSecondsOfRemainingDay = Math.trunc(
    remainingDayDecimal * totalSecondsInDay,
  );
...
function getCurrentSolsticeOrEquinox(currentDateTime, season) {
  ...
  function sumSolsticeEquinoxTable(T) {
    ...
    const sum = sumSolsticeEquinoxTableHelper(T, 485, 324.96, 1934.136) +
  ...
  if (year > 999) {
    ...
    const DeltaLambda = 1 + (0.0334 * Math.cos(W * Math.PI / 180)) +
...
function getLongitudeOfSun(currentDateTime) {
  ...
  const L = normalizeAngleTo360(
    280.46645 + 36000.76983 * T + 0.0003032 * T ** 2,
  );
  const M = normalizeAngleTo360(
    357.52910 + 35999.05030 * T - 0.0001559 * T ** 2 - 0.00000048 * T ** 3,
  );
  const C = +(1.914600 - 0.004817 * T - 0.000014 * T ** 2) *
...
function getNewMoonThisMonth(currentDateTime, monthModifier) {
  ...
  function sumNewMoonTable(SunM, MoonM, F, E, lunarNode) {
    ...
    const sum = sumNewMoonTableHelper(-0.40720, MoonM) +
  ...
  const JDE = 2451550.09765 + 29.530588853 * k + 0.0001337 * T ** 2 +
    -0.000000150 * T ** 3 + 0.00000000073 * T ** 4;
  const SunM = 2.5534 + 29.10535669 * k - 0.0000218 * T ** 2 -
    0.00000011 * T ** 3;
  const MoonM = 201.5643 + 385.81693528 * k + 0.0107438 * T ** 2 +
    0.00001239 * T ** 3 - 0.000000058 * T ** 4;
  const F = 160.7108 + 390.67050274 * k - 0.0016341 * T ** 2 -
    0.00000227 * T ** 3 + 0.000000011 * T ** 4;
  const lunarNode = 124.7746 - 1.56375580 * k + 0.0020691 * T ** 2 +
    0.00000215 * T ** 3;
