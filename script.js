...
function updateDateAndTime(dateInput, firstPass) {
  ...
  if (
    (currentDateTime.getMilliseconds() < millisecondStart) ||
    (currentPass === 100)
  ) {
  ...
  setTimeValue(
    "minute-node",
    calculateMinute(currentDateTime).toFixed(decimals),
  );
  setTimeValue(
    "second-node",
    calculateSecond(currentDateTime).toFixed(decimals),
  );
  setTimeValue(
    "decade-node",
    calculateDecade(currentDateTime).toFixed(decimals),
  );
  setTimeValue(
    "century-node",
    calculateCentury(currentDateTime).toFixed(decimals),
  );
  if ((currentDateTime.getMilliseconds() > 900) || (currentPass === 100)) {
    setTimeValue(
      "millennium-node",
      calculateMillennium(currentDateTime).toFixed(decimals),
    );
  }
  ...
  if (
    ...
    setTimeValue(
      "tai-node",
      getTAI(currentDateTime).toISOString().slice(0, -5),
    );
    setTimeValue(
      "loran-c-node",
      getLORANC(currentDateTime).toISOString().slice(0, -5),
    );
    setTimeValue("lilian-date-node", getLilianDate(julianDay));
    setTimeValue("mars-sol-date-node", marsSolDay.toFixed(0));
    setTimeValue(
      "julian-sol-number-node",
      getJulianSolDate(marsSolDay).toFixed(0),
    );
    setTimeValue(
      "kali-ahargaṅa-node",
      getKaliAhargana(currentDateTime).toFixed(0),
    );
  }
  ...
  // Solar Calendars
  if (
    (((currentDateTime.getMilliseconds() > 500) &&
      (currentDateTime.getMilliseconds() < 510)) && (currentPass === 5)) ||
    (currentPass === 100)
  ) {
    ...
    setTimeValue(
      "french-republican-node",
      getRepublicanCalendar(
        currentDateTime,
        getCurrentSolsticeOrEquinox(currentDateTime, "autumn"),
      ),
    );
    ...
    setTimeValue(
      ...
      getBahaiCalendar(
        currentDateTime,
        getCurrentSolsticeOrEquinox(currentDateTime, "spring"),
      ),
    );
  ...
  if (
    ...
    setTimeValue(
      "mayan-long-count-node",
      getCurrentMayanLongCount(currentDateTime),
    );
    setTimeValue(
      "darian-node",
      getDarianCalendar(getJulianSolDate(marsSolDay)),
    );
  }
  ...
  if (
    (((currentDateTime.getMilliseconds() > 500) &&
      (currentDateTime.getMilliseconds() < 510)) && (currentPass === 1)) ||
    (currentPass === 100)
  ) {
    let lunisolarCalendarChina = getLunisolarCalendarDate(currentDateTime, 16); // China midnight happens at UTC 16:00
    let chineseCalendar = getChineseLunisolarCalendarDate(
      currentDateTime,
      lunisolarCalendarChina,
    );
  ...
  if (
    (((currentDateTime.getMilliseconds() > 500) &&
      (currentDateTime.getMilliseconds() < 510)) && (currentPass === 2)) ||
    (currentPass === 100)
  ) {
    let lunisolarCalendarVietnam = getLunisolarCalendarDate(
      currentDateTime,
      15,
    ); // Vietnam midnight happens at UTC 15:00
    setTimeValue(
      "vietnamese-node",
      getVietnameseLunisolarCalendarDate(
        currentDateTime,
        lunisolarCalendarVietnam,
      ),
    );
  }
  if (
    (((currentDateTime.getMilliseconds() > 500) &&
      (currentDateTime.getMilliseconds() < 510)) && (currentPass === 3)) ||
    (currentPass === 100)
  ) {
    let lunisolarCalendarKorea = getLunisolarCalendarDate(currentDateTime, 17); // Korea midnight happens at UTC 17:00
    setTimeValue(
      "dangun-node",
      getDangunLunisolarCalendarDate(currentDateTime, lunisolarCalendarKorea),
    );
  }
  if (
    (((currentDateTime.getMilliseconds() > 500) &&
  ...
  if (
    (((currentDateTime.getMilliseconds() > 500) &&
      (currentDateTime.getMilliseconds() < 510)) && (currentPass === 4)) ||
    (currentPass === 100)
  ) {
  ...
  if (
    (((currentDateTime.getMilliseconds() > 500) &&
      (currentDateTime.getMilliseconds() < 510)) && (currentPass === 6)) ||
    (currentPass === 100)
  ) {
    setTimeValue(
      "spring-equinox-node",
      getCurrentSolsticeOrEquinox(currentDateTime, "spring").toUTCString(),
    );
    setTimeValue(
      "summer-solstice-node",
      getCurrentSolsticeOrEquinox(currentDateTime, "summer").toUTCString(),
    );
    setTimeValue(
      "autumn-equinox-node",
      getCurrentSolsticeOrEquinox(currentDateTime, "autumn").toUTCString(),
    );
  }
  if (
    (((currentDateTime.getMilliseconds() > 500) &&
      (currentDateTime.getMilliseconds() < 510)) && (currentPass === 7)) ||
    (currentPass === 100)
  ) {
    setTimeValue(
      "winter-solstice-node",
      getCurrentSolsticeOrEquinox(currentDateTime, "winter").toUTCString(),
    );
    setTimeValue(
      "sun-longitude-node",
      getLongitudeOfSun(currentDateTime) + "°",
    );
    setTimeValue(
      "this-new-moon-node",
      getNewMoonThisMonth(currentDateTime, 0).toUTCString(),
    );
  }
  ...
  // Politics
  setTimeValue(
    "us-presidential-terms-node",
    getCurrentPresidentialTerm(currentDateTime).toFixed(10),
  );
}
...
function createnode(item) {
  ...
  epochElement.innerHTML =
    `<table class="table-epoch"><tr><th><b>Epoch</th><tr><td>${item.epoch}</b></td></tr></tr></table>`;
  epochElement.classList.add("tooltip-epoch");

  const confidenceElement = document.createElement("div");
  confidenceElement.innerHTML =
    `<table class="table-confidence"><tr><th><b>Confidence: ${item.confidence}</b></th></tr></table>`;
  ...
  document.querySelector(".description-wrapper").appendChild(
    overviewDescription,
  );
...
function changeHeaderButton(button, index) {
  ...
  buttons.forEach((btnId) => {
    ...
    if (isSelected) {
      ...
      for (let i = 0; i < currentDescriptionPage.length; i++) {
        currentDescriptionPage[i].style.visibility = i === index
          ? "visible"
          : "hidden";
      }
    }
