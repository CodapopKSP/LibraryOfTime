...
function calculateSecond(currentDateTime) {
  ...
  let secondFraction = (currentDateTime - secondStart) /
    (nextSecondStart - secondStart);
...
function calculateMinute(currentDateTime) {
  ...
  let minuteFraction = (currentDateTime - minuteStart) /
    (nextMinuteStart - minuteStart);
...
function calculateHour(currentDateTime) {
  ...
  let hourFraction = (currentDateTime - hourStart) /
    (nextHourStart - hourStart);
  return hourFraction;
}
...
function calculateMonth(currentDateTime) {
  ...
  let monthFraction = (currentDateTime - monthStart) /
...
function calculateYear(currentDateTime) {
  ...
  let yearFraction = (currentDateTime - yearStart) /
...
function calculateDecade(currentDateTime) {
  ...
  let decadeFraction = (currentDateTime - decadeStart) /
    (nextDecadeStart - decadeStart);
...
function calculateCentury(currentDateTime) {
  ...
  let centuryFraction = (currentDateTime - centuryStart) /
    (nextCenturyStart - centuryStart);
...
function calculateMillennium(currentDateTime) {
  ...
  let nextMillenniumStart = new Date(
    Math.trunc(year / 1000) * 1000 + 1000,
    0,
    1,
  );
  nextMillenniumStart.setFullYear(Math.trunc(year / 1000) * 1000 + 1000);
  let millenniumFraction = (currentDateTime - millenniumStart) /
    (nextMillenniumStart - millenniumStart);
