...
function getRevolutionaryTime(dayFraction) {
  ...
  let decimalSecond = Math.trunc(dayFraction * 100000) - (decimalHour * 10000) -
    (decimalMinute * 100);
...
function convertToSwatchBeats(currentDateTime) {
  ...
  let dayFraction =
    ((utcDateTime.getHours() + 1) * 3600 + utcDateTime.getMinutes() * 60 +
      utcDateTime.getSeconds()) / 86400;
...
function getHexadecimalTime(dayFraction) {
  ...
  let hexadecimalFraction = Math.trunc(dayFraction * 65536).toString(16)
    .toUpperCase();
...
function get6DigitHexadecimalTime(dayFraction) {
  // Convert the day fraction to hexadecimal format
  let hexadecimalFraction = Math.trunc(dayFraction * 16777215).toString(16)
    .toUpperCase();
  // Pad the hexadecimal fraction with leading zeros if necessary to ensure four digits
