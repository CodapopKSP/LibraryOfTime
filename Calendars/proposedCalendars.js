...
function getInvariableCalendarDate(currentDateTime) {
  ...
  const daysSinceStartOfYear = Math.trunc(
    (currentDateTime.getTime() - startOfYear.getTime()) /
      (1000 * 60 * 60 * 24),
  ) + 1;
  let daysRemaining = daysSinceStartOfYear;

  // Need two lists for each for Leap Years and non Leap Years
  const monthDaysLeapYear = [
    1,
    30,
    30,
    31,
  ...
  if (
    (invariableMonth === "New Years Day") || (invariableMonth === "Leap Day")
  ) {
    invariableDate = "";
  }
...
function getWorldCalendarDate(currentDateTime) {
  ...
  // Remove the date string if using a named day
  if (
    (invariableMonth === "World's Day") || (invariableMonth === "Leapyear Day")
  ) {
    invariableDate = "";
  }
