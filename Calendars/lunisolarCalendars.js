...
function getChineseLunisolarCalendarDate(currentDateTime, lunisolarDate) {
  ...
  return `${year}年 ${lunisolarDate.month}月 ${lunisolarDate.day}日\nYear of the ${
    zodiacAnimals[earthlyBranchIndex]
  }`;
}
...
function getSexagenaryYear(chineseDate) {
  ...
  return heavenlyStem + earthlyBranch + " (" + heavenlyStemEnglish +
    earthlyBrancheEnglish + ")";
}
...
function getLunisolarCalendarDate(currentDateTime, utcMidnight) {
  ...
  const midnightStartOfMonthEleven = getMidnightInUTC(
    startOfMonthEleven,
    utcMidnight,
  );
  ...
  const winterSolsticeLastYear = getCurrentSolsticeOrEquinox(
    lastYear,
    "winter",
  );
  const startOfMonthElevenLastYear = getMonthEleven(winterSolsticeLastYear);
  const midnightStartOfMonthElevenLastYear = getMidnightInUTC(
    startOfMonthElevenLastYear,
    utcMidnight,
  );

  // Find out roughly how many months between solstices
  const daysBetweenEleventhMonths = Math.trunc(
    (midnightStartOfMonthEleven - midnightStartOfMonthElevenLastYear) / 1000 /
      60 / 60 / 24,
  );
  const lunationsBetweenEleventhMonths = Math.round(
    daysBetweenEleventhMonths / 29.53,
  );
  let currentMonth = 0;
  ...
  if (lunationsBetweenEleventhMonths === 12) {
    ...
    const midnightChinaStartOfMonth = getMidnightInUTC(
      startofThisMonth,
      utcMidnight,
    );
    const startofLastMonth = getNewMoonThisMonth(currentDateTime, -1);
    const midnightChinaStartOfLastMonth = getMidnightInUTC(
      startofLastMonth,
      utcMidnight,
    );
    const daysSinceMonthEleven =
      (currentDateTime - midnightStartOfMonthEleven) / 1000 / 60 / 60 / 24;
    ...
    currentDay = Math.trunc(
      (currentDateTime - midnightChinaStartOfMonth) / 1000 / 24 / 60 / 60,
    ) + 1;

    // If the current day is less than 1, then it's the previous month
    if (currentDay < 1) {
      currentDay = Math.trunc(
        (currentDateTime - midnightChinaStartOfLastMonth) / 1000 / 24 / 60 /
          60,
      ) + 1;
    }
  ...
  if (lunationsBetweenEleventhMonths === 13) {
    ...
    const daysSinceMonthEleven =
      (currentDateTime - midnightStartOfMonthEleven) / 1000 / 60 / 60 / 24;
    const leapMonth = calculateFirstMonthWithoutMajorSolarTerm(
      midnightStartOfMonthElevenLastYear,
    );
...
function getSolarTermTypeThisMonth(startOfMonth) {
  ...
  let newMoonNextMonth = new Date(
    newMoonThisMonth.getTime() + millisecondsIn29_53Days,
  );
  ...
  for (const term of MajorSolarTerms) {
    ...
    if (
      term > newMoonThisMonthLongitudeOfSun &&
      term < newMoonNextMonthLongitudeOfSun
    ) {
      return "major";
    }
  }
...
function calculateFirstMonthWithoutMajorSolarTerm(
  midnightStartOfMonthElevenLastYear,
) {
...
function calculateHebrewCalendar(currentDateTime) {
  ...
  const Hebrew_monthDaysDeficient = [
    30,
    29,
    29,
    29,
  ...
  const Hebrew_monthDaysComplete = [
    30,
    30,
    30,
    29,
  ...
  let remainingDays = Math.trunc(
    (currentDateTime - lastTishri) / 1000 / 24 / 60 / 60,
  );
  ...
  const Hebrew_monthDays = isDeficientYear
    ? Hebrew_monthDaysDeficient
    : isRegularYear
    ? Hebrew_monthDaysRegular
  ...
  return hebrewDate.day + " " + HebrewMonths[hebrewDate.month] + " " +
    hebrewDate.year + " AM";
}
...
function getStartOfTishri(currentDateTime) {
  ...
  if ((dayOfWeekOfTishri1 % 1) > 0.5) {
    ...
    (Math.trunc(dayOfWeekOfTishri1) === 3) &&
    ((dayOfWeekOfTishri1 % 1) > 0.383) &&
    !(isMetonicCycleLeapYear(Math.trunc(yearsThisMetonicCycle)))
  ) {
    ...
    (Math.trunc(dayOfWeekOfTishri1) === 2) &&
    ((dayOfWeekOfTishri1 % 1) > 0.6478) &&
    (isMetonicCycleLeapYear(Math.trunc(yearsThisMetonicCycle - 1)))
  ) {
  ...
  if (
    Math.trunc(dayOfWeekOfTishri1) === 1 ||
    Math.trunc(dayOfWeekOfTishri1) === 4 || Math.trunc(dayOfWeekOfTishri1) === 6
  ) {
  ...
  const millisecondsSinceMoladTishri5732 = (daysFromMoladTishri5732 - 1) * 24 *
    60 * 60 * 1000;
  // Get sunset of Tishri 1
  let startOfTishri = new Date(
    moladTishri5732.getTime() + millisecondsSinceMoladTishri5732,
  );
