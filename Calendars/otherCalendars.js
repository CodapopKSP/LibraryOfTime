...
function getCurrentMayanLongCount(currentDateTime) {
  ...
  const daysSinceStart = Math.floor(
    (currentDateTime - mayanStartDate) / (1000 * 60 * 60 * 24),
  );
...
function getDarianCalendar(julianSolNumber) {
  ...
  const daysOfMonths = isLeapYear(year)
    ? daysOfMonthsLeapYear
    : daysOfMonthsNonLeapYear;
  let month = 0;
