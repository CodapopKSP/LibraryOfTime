...
2. Create a **new markdown file** with:
   - A title and basic outline/section headings (e.g., name, epoch, overview).
   - **No detailed content** unless the user explicitly requested text (respect the project’s `Docs/` rule).
...
3. For initial project setup, the stub **does not need to perform real calendar calculations**:
...
4. When you later implement the real calculation, then apply `calendar-math` and `calendar-conventions` (anchor dates, timezone handling, helpers, etc.) without changing the public signature if possible.
